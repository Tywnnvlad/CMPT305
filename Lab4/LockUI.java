...
import java.util.*;
/**
 * This class will create a User interface for the user. where the user can interact with.
 */
public class LockUI extends JFrame {
...
/**
 * This function is the one that is responsible for creating the UI for the lock
 * 
 * 
 */
public void GenerateUI (){
    ...
    setPreferredSize(d);
    //BUTTON panel, it is made of all the panels for the buttons
    ...
    //STRING panel, the place where the input will be placed
    JPanel p2 = new JPanel();
    p2.setLayout(new FlowLayout());
    p2.add(jtextfield = new JTextField(30));
    jtextfield.setHorizontalAlignment(JTextField.RIGHT);
    jtextfield.setEditable(false);
    //ACTION panel, the place where the user can enter or delete the input
    ...
    //Add different kind of listeners for the buttons
    ...
    class ListenEnter implements ActionListener {
        public void actionPerformed(ActionEvent e) {
            String pw = Lock.getPassword();
            display = jtextfield.getText();
            if(display.equals(pw)){
                jtextfield.setText("SUCCESFUL!!");
            }
            else {
                jtextfield.setText("WRONG PASSWORD!!");
            }
        }
    }
